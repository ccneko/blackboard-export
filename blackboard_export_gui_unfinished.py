...
def bbdl_gui(): # unfinished, to write into class
    ...
    w.setWindowTitle('BlackBoard Downloader') #unfinished, to output log
    user_eid, ok = QInputDialog.getText(w, "Get text","Please input your user ID:", QLineEdit.Normal, "")
    user_password, ok = QInputDialog.getText(w, "Attention", "Please input your password:", QLineEdit.Password)
    QMessageBox.question(w, 'PyQt5 message', "I am under development. Please use non-GUI version for now. Thanks!", QMessageBox.Yes | QMessageBox.No, QMessageBox.No) 
    w.show()
    sys.exit(app.exec_())
    return (user_eid, user_password)

# To write message dialog box
...
def main():
    ...
    user_eid,user_password = bbdl_gui()
